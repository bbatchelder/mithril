...
// ── registry.json: distribution + the per-component page docs (title, description,
//    registry/npm dependencies). Keyed by the ui-component id its files map to. ──────
const registry = JSON.parse(readFileSync(join(ROOT, "registry.json"), "utf8"));
const inRegistry = new Set();
const regItemById = new Map();
for (const item of registry.items ?? []) {
    for (const file of item.files ?? []) {
        const m = /src\/components\/ui\/([a-z0-9-]+)\.tsx$/.exec(file.path ?? "");
        if (m) {
            inRegistry.add(m[1]);
            regItemById.set(m[1], item);
        }
    }
}
