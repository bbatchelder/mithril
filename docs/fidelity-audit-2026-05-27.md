# Visual fidelity audit — mithril vs Blueprint reference
...
- **Observed:** For bottom-placed overlays, mithril renders the arrow as a **downward chevron floating in the gap** between trigger and content, and the content sits with a visibly larger gap below the trigger. Blueprint renders the arrow **pointing up, attached to the content's top edge**, tucked snug under the trigger.
...
- **Eyeball:** `popover.dark.{mithril,blueprint}.png`, `tooltip.dark.{mithril,blueprint}.png`.

### 2. Switch with inner labels — **High**
- **Specimen:** `switch-inner-labels` (0.282 light / 0.649 dark), 1px size flag.
- **Observed:** With inner labels, mithril's track isn't wide enough — `OFF` renders clipped/overlapping the thumb (a smudged `O̶FF`). Blueprint widens the track so `OFF`/`ON` sits clearly beside the thumb.
...
- **Eyeball:** `switch.light.{mithril,blueprint}.png` — "Inner labels" section.
...
- **Observed:** The calendar **body** (day grid, selected-day highlight, range highlight) matches Blueprint well in the popover contexts. The **header** differs consistently:
  - Month/year pickers render as **bordered `HTMLSelect` boxes** in mithril; Blueprint uses **borderless minimal selects** (label + small up/down caret, no box).
  - Nav arrows are pushed to the calendar's outer edges (`justify-content: space-between`) in mithril; Blueprint groups `[month][year][<][>]` together.
...
- **Eyeball:** `date-picker.light.{mithril,blueprint}.png` (clearest), then `date-input` / `date-range-input` for the header treatment.

### 4. Hotkeys / KeyCombo — **Med**
- **Specimens:** `hotkey-key` (0.138 light), `hotkey-combo` (0.223 dark), 3px size flag.
- **Observed:** Blueprint renders each modifier keycap as **glyph + word** (`⌘ cmd`, `⇧ shift`, `^ ctrl`). Mithril shows only the word (`cmd`, `shift`) and renders some modifiers as glyph-only caps — the platform glyph is missing from the labelled caps, so caps are narrower.
...
- **Eyeball:** `hotkeys.light.{mithril,blueprint}.png` — both the top "KeyCombo:" row and the dialog rows.

### 5. Slider — **Med**
- **Specimens:** `slider-success` (0.427 light / 0.578 dark), `slider-primary`, etc. (6 flagged).
- **Observed:** Two issues — (a) the value-label pill hangs **lower** with a gap below the handle and **collides with the axis tick labels** (0/5/10 row); Blueprint tucks it tight under the handle aligned to that row. (b) The handle reads as a **bordered rounded-square** in mithril vs Blueprint's cleaner round knob.
...
- **Eyeball:** `slider.light.{mithril,blueprint}.png`.

### 6. FileInput default width — **Med (verify)**
- **Specimens:** `fi-large` (0.755 light / 0.739 dark), 4px size flag.
- **Observed:** mithril's FileInput stretches to **full container width** (~760px, Browse pushed far right) on the non-fill variants, while Blueprint's default is **inline ~250px**. (The explicit "Fill" row in mithril is paradoxically narrower.)
...
- **Eyeball:** `file-input.light.{mithril,blueprint}.png`.
...
- `multi-select-tag` (0.008 dark / 0.035 light) is inflated by a 1px input-row height offset; the **chips match**. The real diff: mithril draws the active menu item ("Apple") with a blue **outline ring**; Blueprint fills it with a muted blue background. Align the active-item treatment to the filled highlight (as Select already does). `multi-select.dark.{mithril,blueprint}.png`.

### 8. NumericInput stepper — **Low**
- `ni-step-button` (0.828 dark / 0.855 light), 6px size flag. Stepper group sits with a small gap / slightly detached from the field and is ~6px off in width; Blueprint's stepper is flush against the input as one unit. Intent-colored steppers otherwise match. `numeric-input.light.{mithril,blueprint}.png`.

### 9. Select trigger — **Low (verify)**
- `select-trigger` (0.54), 5px size flag. mithril's trigger hugs content (`Cherry ▾`, caret adjacent, ~80px); Blueprint's is full-width (~300px) with the caret pinned right via space-between. The dropdown list matches well. Likely a gallery-config difference (reference button set to `fill`) — confirm before treating as a component gap. `select.light.{mithril,blueprint}.png`.

### 10. TagInput chips — **Low**
- `tag-input-container` (0.733/0.735), 2px size flag. Chips render slightly wider — more space before the × remove icon. (The "left icon" demo also uses a *search* icon in mithril vs a *tag* icon in Blueprint — gallery content, not a gap.) `tag-input.light.{mithril,blueprint}.png`.
...
## Tier 4 — benign noise / reference-gallery artifacts (NOT mithril gaps)

These flagged in the ranking but are not real mithril fidelity gaps. Several point at the
**reference gallery / harness**, worth a separate cleanup pass:

- **panel-stack** (0.007/0.009) — **reference artifact.** The Blueprint reference rendered an **empty panel** (content didn't mount), so the crop compares mithril's correct header/content against blank. mithril is fine. → *fix the reference gallery's PanelStack mount.*
...
- **timezone-select** (0.144/0.151) — the two galleries render **different timezone datasets**, so per-item crops compare mismatched rows. Styling/layout faithful. (Minor real note: mithril auto-highlights the first row; Blueprint shows none.) → *align gallery data for a clean signal.*
...
- **context-menu / menu** (0.79 / 0.746) — **bare-surface width artifact.** The reference renders a bare `Menu` in a full-width container (so ⌘F right-aligns to the page edge); mithril's bare Menu stays content-width. In real popovers (Select/timezone/multi-select) mithril's menu fills and right-aligns correctly. Style diffs are trivial (`min-width: auto` vs `0`, 1px heights).
...
- **dialog / drawer** (0.836/0.81) — only style diff is the 1px box-shadow ring color `rgb(0,0,0)` vs `rgb(20,20,20)` at ~10% opacity — imperceptible. `only in mithril: drawer-close` = a **reference-gallery tagging gap** (key not paired). → *tag `drawer-close` in the reference, or drop the tag.*
...
2. **`min-width: auto` vs `0`** appears on menu/segmented-control/etc. — mithril's default; visually inert, but normalizing to `0` would silence several style-diff flags.
...
4. **Reference-gallery cleanup** (separate from mithril): PanelStack empty mount, timezone dataset mismatch, untagged `drawer-close` — these distort the ranking, not the product.
