 ...
 *
 * Button tiers match `:where(button, .mithril-button)` so they apply to a raw <button>, a
 * Button, an `asChild` Button, and an AnchorButton alike (the last two render <a> but carry
 * the `.mithril-button` marker from `buttonVariants`). Intent tiers key off the `data-intent`
...
const controlGroupVariants = cva(
    [
        ...
        // Z-index stacking — buttons. `:where(button, .mithril-button)` matches a raw <button>,
        // a mithril Button, an `asChild` Button, and an AnchorButton (the latter two render
        // <a>, not <button>, but carry the `.mithril-button` marker). `:where()` adds zero
        ...
        // disabled button: z-3
        "[&_:where(button,.mithril-button):disabled]:z-[3]",
        // default button: z-4
        "[&_:where(button,.mithril-button)]:z-[4]",
        // focused/hovered/active button: z-5
        "[&_:where(button,.mithril-button):focus]:z-[5]",
        "[&_:where(button,.mithril-button):hover]:z-[5]",
        "[&_:where(button,.mithril-button):active]:z-[5]",
        ...
        "[&_:where(button,.mithril-button)[data-intent]:not(:disabled)]:z-[6]",
