...
describe("axe smoke — inline components", () => {
    ...
    it("AILabel (static marker)", async () => {
        const { container } = render(<AILabel />);
        expect(await axe(container)).toHaveNoViolations();
    });

    it("AILabel (icon-only needs an accessible name)", async () => {
        const { container } = render(<AILabel label={null} ariaLabel="AI generated" />);
        expect(await axe(container)).toHaveNoViolations();
    });

    it("AILabel (interactive trigger)", async () => {
        const { container } = render(
            <AILabel popover={<AILabelExplanation>Generated by AI.</AILabelExplanation>} />,
        );
        expect(await axe(container)).toHaveNoViolations();
    });
