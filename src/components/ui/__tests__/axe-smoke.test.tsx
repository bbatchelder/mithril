...
import { AIExplainability, AIExplainabilityDetails } from "../ai-explainability";
...
describe("axe smoke — inline components", () => {
    ...
    it("AIExplainability (static marker)", async () => {
        const { container } = render(<AIExplainability />);
        expect(await axe(container)).toHaveNoViolations();
    });

    it("AIExplainability (icon-only needs an accessible name)", async () => {
        const { container } = render(<AIExplainability label={null} ariaLabel="AI generated" />);
        expect(await axe(container)).toHaveNoViolations();
    });

    it("AIExplainability (interactive trigger)", async () => {
        const { container } = render(
            <AIExplainability popover={<AIExplainabilityDetails>Generated by AI.</AIExplainabilityDetails>} />,
        );
        expect(await axe(container)).toHaveNoViolations();
    });
